...
let execute = async (interaction) => {
  ...
  let response = await axios.get(
    `http://127.0.0.1:5000/battleimg?pokemon1=${name1}&pokemon2=${name2}&rem1=${hp1}&rem2=${hp2}`
  );
    ...
    embeds: [
      {
        ...
        color: "RANDOM",
        fields: [
          { name: `Pokemon1: ${name1}`, value: `HP: ${hp1.toString()}` },
          { name: `Pokemon2: ${name2}`, value: `HP%: ${hp2.toString()}` },
        ],
      },
    ],
    files: [file],
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("pokemon1name")
        .setDescription("Pokemon1 name")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName("pokemon2name")
        .setDescription("Pokemon2 name")
        .setRequired(true)
    )
    .addIntegerOption((option) =>
      option
        .setName("remhp1")
        .setDescription("Pokemon1 remaining HP percentage")
        .setRequired(true)
    )
    .addIntegerOption((option) =>
      option
        .setName("remhp2")
        .setDescription("Pokemon2 remaining HP percentage")
        .setRequired(true)
    ),
  execute: execute,
};
